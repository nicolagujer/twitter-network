...
var nfilter = crossfilter(), 
    efilter = crossfilter();
var nodesDegDim, edgesWeightDim, junctionsWeightDim, followersDegDim, favDegDim;
...
function group_auto(data) {
    ...
    /*var groups = d3.set(data.neurons.map(function(d) { return d.twitter;} ).sort()).values();
    $(function () {
        $("#group1, #group2")
    ...
    var groups = d3.set(data.neurons.map(function(d) { return d.twitter;} ).sort()).values();
    $(function () {
        $("#group1, #group2").autocomplete({source: groups});
    });
    ...
    var institutes = d3.set(data.neurons.map(function(d) { return (d.inst3 || d.inst2 || d.type);} ).sort()).values();
    $(function () {
        $("#search-inst").autocomplete({source: institutes});
    });

    /*var inst2 = data.neurons
        .filter(function(x) { return "inst2" in x; })
        .map(function(d) { return d.inst2.split(", "); });
    inst2 = [].concat.apply([], inst2).sort(); // flatten array
    inst2 = d3.set(inst2).values();
    $(function () {
        $("#rec-sel")
        .bind("keydown", function(event) {
            if (event.keyCode === $.ui.keyCode.TAB && $(this).autocomplete( "instance" ).menu.active) {
        ...
        .autocomplete({
            minLength: 0,
            source: function( request, response ) {
                response($.ui.autocomplete.filter(inst2, extractLast(request.term)));
            },
            focus: function() { return false; }, // prevent value inserted on focus
            select: function( event, ui ) {
                var terms = split( this.value );                
...
graph = function(id, d, canvas) {
    ...
    data = d;
    use_canvas = canvas;
    neuronTypes = d3.set(data.neurons.map(function(d) { return (d.inst3 || d.inst2 || d.type);} ).sort()).values();
    ...
    //colors = [col_inter, col_motor, col_intmot, col_sensory, col_sensint, col_sensmot];
    //colors = ["#a6cee3", "#b2df8a", "#fb8072","#fb9a99", "#e31a1c", "#0404B4","#fdbf6f", "#00FFFF","#ff7f00", "#cab2d6", "#6a3d9a", "#33ccff", "#00cc00", "#ff80bf", "#bf80ff", "#b30059"]
    colors = ["#59b300", "#8A2BE2", "#009999", "#ff99ff", "#1E90FF", "#47E797", "#9966ff", "#ffcc00", "#009999", "#6495ED", "#ff8b66", "#990099", "#9932CC", "#00BFFF", "#FF8C00", "#FF1493", "#FFD700", "#008000", "#FF69B4", "#FFB6C1", "#20B2AA", "#87CEFA", "#C71585", "#59b300", "#8A2BE2", "#009999", "#ff99ff", "#47E797", "#9966ff", "#ffcc00", "#FF7F50", "#6495ED", "#DC143C", "#008B8B", "#FF69B4", "#9932CC", "#00BFFF", "#FF1493", "#FFD700", "#FF8C00", "#008000", "#1E90FF", "#ff0027", "#87CEFA", "#C71585", "#ff80b2", "#009933", "#b32c00", "#bb99ff", "#8080ff", "#cbcc00"];
    nodeColorScale = d3.scale.ordinal().domain(neuronTypes).range(colors);
    //nodeColorScale = d3.scale.category20()
    
    var degreeDomain = d3.extent(data.neurons, function(n) { return Math.min(Math.max(5,n.NumTweets),20000); });
    nodeRadiusScale = d3.scale.log().domain(degreeDomain).range([10,30]);
    ...
    nodesDegDim = nfilter.dimension(function(d) { return d.NumTweets; });
    followersDegDim = nfilter.dimension(function(d) { return d.followers; });
    favDegDim = nfilter.dimension(function(d) { return d.FavCount; });
    ...
    filter(ndegVal, foldegVal, favdegVal, wminVal, jminVal);
...
function updateCrossFilter(n, s) {
    ...
    followersDegDim.filter(null);
    favDegDim.filter(null);
...
function filterFolDeg(foldeg) {
    foldegVal = foldeg;
    document.querySelector('#foldeglabel').value = foldeg;
    filter(ndegVal, foldegVal, favdegVal, wminVal, jminVal);
}

function filterFavDeg(favdeg) {
    favdegVal = favdeg;
    document.querySelector('#favdeglabel').value = favdeg;
    filter(ndegVal, foldegVal, favdegVal, wminVal, jminVal);
}
...
function filter(ndeg, foldeg, favdeg, wmin, jmin) {

    var prune = d3.select("#prune1").classed("active");

    if (typeof ndeg == 'undefined') ndeg = ndegVal; //$( "#nsl_slider" ).slider( "value" );
    if (typeof foldeg == 'undefined') foldeg = foldegVal;
    if (typeof favdeg == 'undefined') favdeg = favdegVal;
    ...
    followersDegDim.filter([foldeg, Infinity]);
    var fol = followersDegDim.top(Infinity);
    nodeIds = d3.set(fol.map(function(d) { return d.id; }));

    favDegDim.filter([favdeg, Infinity]);
    var fav = favDegDim.top(Infinity);
    nodeIds = d3.set(fav.map(function(d) { return d.id; }));
    ...
    var ncon = nfilter.dimension(function(d) {return edgeIds.has(d.id);});
    var folcon = nfilter.dimension(function(d) {return edgeIds.has(d.id);});
    var favcon = nfilter.dimension(function(d) {return edgeIds.has(d.id);});
    
    ncon.filter(function(d) { return d; });
    n = ncon.top(Infinity);
    ncon.dispose();

    folcon.filter(function(d) {return d; });
    fol = folcon.top(Infinity);
    folcon.dispose();

    favcon.filter(function(d) {return d; });
    fav = favcon.top(Infinity);
    favcon.dispose();
    ...
    var optArray2 = n.map(function(d) { return (d.inst3 || d.inst2 || d.type);} ).sort();
    $(function () {
        $("#search").autocomplete({source: optArray2});
    });
...
/*function htmlForNode(d){
    var str = 
        "Twitter Handle: " + d.twitter + "<br>" +
        "Institution: " + d.type + "<br>" +
        "Number of Favourites: " + d.FavCount + "<br>" +
        "Degrees:" + "<br>&emsp;" + "in " + d.followers + " out " + d.following + " total " + d.NumTweets + "<br>";
    return str;
} */

function htmlTabForNode(d){
    col = nodeColorScale(d.type);
    inst2col = nodeColorScale(d.inst2);
    inst3col = nodeColorScale(d.inst3);
   ...
   var str = '<p><span class="badge stats-item" style="background-color:'+col+'">' + d.type + '</span>'
    ...
    if ("inst2" in d) {
        ...
        for (var i = mods.length - 1; i >= 0; i--) {
            str += '<span class="badge badge-sm stats-item" style="background-color:'+inst2col+'">' + mods[i] + '</span>';
        };
    }
    ...
    if ("inst3" in d) {
        mods = d.inst3.split(", ");  
        for (var i = mods.length - 1; i >= 0; i--) {
            str += '<span class="badge badge-sm stats-item" style="background-color:'+inst3col+'">' + mods[i] + '</span>';
        };
    }
    ...
    if ("description" in d && d.description != '') {
        str +=  '<p><b>Description</b>: ' + d.description + '</p>';
    }
    ...
    if (d.type == 'muscle') {
        str +=
            ...
            '<li class="list-group-item"><span class="badge stats-item">' + d.followers + '</span>In Degree</li>';    
    }
    else {
        str +=
            '<li class="list-group-item"><span class="badge stats-item" >' + d.twitter + '</span>Twitter Handle</li>' +
            '<li class="list-group-item"><span class="badge stats-item">' + d.FavCount + '</span>Favourites Count</li>' +
            '<li class="list-group-item"><span class="badge stats-item">' + d.followers + '</span>Followers Count</li>' +
            '<li class="list-group-item"><span class="badge stats-item">' + d.following + '</span>Following Count</li>' +
            '<li class="list-group-item"><span class="badge stats-item">' + d.NumTweets + '</span>Number of Tweets</li>' +
            /*'<li class="list-group-item"><span class="badge stats-item">' + d.location + '</span>Location</li>' + */
            /*(mods ? ('<li class="list-group-item"><span class="badge stats-item">' + d.organ + '</span>Other Place Holder</li>') : '') + */
        
            '<li class="list-group-item"><a target="_blank" href="http://twitter.com/' + d.twitter +  
                '"><span class="glyphicon glyphicon-new-window pull-right"></span>Twitter </a></li>';
    }
    str += '</ul>'
    return str;
...
drawCanvas = function() {
    ...
    if (highlighted.length == 0 && links.length > numLinksDrawThresh) {
        ...
        context.strokeStyle = "rgb(229, 229, 255)";
        context.beginPath();
        links.forEach(function (l) {    
...
collide = function(alpha) {
    ...
    return function(d) {
        var radius = nodeRadiusScale(d.NumTweets);
...
function searchInst() {
    var selectedInst = document.getElementById('search-inst').value;
    var sel = node.filter(function(d) { return (d.inst3 == selectedInst || d.inst2 == selectedInst || d.type == selectedInst); });
    for (var i = sel.length - 1; i >= 0; i--) {
        if(sel[i].length > 0) {
        nodeClicked(sel.data()[i]);
        connectedNodes(sel.data()[i]);        
        }
    else {
        $('#search-inst').addClass("alert-danger");
    }
}
}
...
function addNodeRadius(neurons) {
   neurons.forEach(function(d) { d.r = nodeRadiusScale(d.NumTweets+1); });
}
...
function expand() {
    if (fetched) {
        ...
        $.getJSON($SCRIPT_ROOT + '/_expand', {
            ...
            setSlider("wmin", wminVal=0);
            setSlider("ndeg", ndegVal=0);
            setSlider("foldeg", foldegVal=0);
            setSlider("favdeg", favdegVal=0);
            updateCrossFilter(data['neurons'], data['synapses']);
...
function subGraph() {
    ...
    $.getJSON($SCRIPT_ROOT + '/_subgraph', {
        ...
        data = d.result;
        jmin = wmin = ndeg = foldeg = favdeg = 0;
        setSlider("jmin", jminVal=0);
        setSlider("wmin", wminVal=0);
        setSlider("favdeg", favdegVal=0);
        setSlider("foldeg", favdegVal=0)
        setSlider("ndeg", ndegVal=0);
        if (highlightId != -1){
            d = nodes.filter(function(n) { return n.id == highlightId; })[0];
            nodeClicked(d);
